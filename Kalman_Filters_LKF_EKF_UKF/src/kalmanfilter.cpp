...
void KalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
    if(isInitialised())
    {
        ...
        VectorXd y_bar_k = z - H*state;

        // innovation cov
        MatrixXd S_k = H*cov*H.transpose()+R;

        //Kalman gain
        MatrixXd K_k = cov*H.transpose()*S_k.inverse();

        // state update
        state = state + K_k*y_bar_k;

        // Covariance update
        cov = (MatrixXd::Identity(4,4) - K_k*H) * cov;
